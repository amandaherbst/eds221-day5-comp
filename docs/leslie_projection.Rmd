...
# The line below creates a sequence of 9 values for time in years (a cycle)
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below creates an empty matrix that is 4 columns and 9 rows, [9,4] (dimensions based on initial conditions and time sequence)
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below fills in the first row of the matrix, proj_year, with the initial population conditions
proj_year[1, ] <- fish_year0

# The for loop below fills in the rest of the proj_year matrix by multiplying matrices 
...
# this creates a data structure that makes it much easier to automate stacked plots as we see below
